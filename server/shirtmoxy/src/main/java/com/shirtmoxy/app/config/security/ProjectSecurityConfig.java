	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		...
		http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				...
				.csrf((csrf) -> csrf.csrfTokenRequestHandler(requestHandler)
						.ignoringRequestMatchers("/contact", "/sign-up", "/subscribe", "/checkout/**") // @TODO
						.csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse()))
				.addFilterAfter(new CsrfCookieFilter(), BasicAuthenticationFilter.class)
				.authorizeHttpRequests((requests) -> requests.requestMatchers("/user/projects").hasRole("USER")
						.requestMatchers("/user/profile").hasRole("USER").requestMatchers("/user/password-reset")
						.hasRole("USER").requestMatchers("/user/manage-account").hasRole("USER")
